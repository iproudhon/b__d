 ...
 *   VERBOSE_TEST=1 - Show debug output and full JSON results
 * 
 * Examples:
 ...
 *   VERBOSE_TEST=1 node llm-client.test.js           # Run all tests with verbose output
 */
...
test('LLMClient - Tool execution: should handle web_search with specific query', { skip: process.env.TEST_BRAVE_ONLY === '1' || process.env.TEST_DUCKDUCKGO_ONLY === '1' }, async () => {
    ...
    // Verbose output if VERBOSE_TEST env var is set
    const verbose = process.env.VERBOSE_TEST === '1' || process.env.VERBOSE_TEST === 'true';
...
if (require.main === module) {
    ...
    if (verbose) {
        ...
        console.log('(Set VERBOSE_TEST=1 to see live test results)\n');
    }
